class CustomerSearchTerm
  ...
  def extract_names(search_text)
    search_text.split(' ')
  end

  def build_name_segment(search_term)
    where_clause << case_insensitive_search(:first_name)
    where_args[:first_name] = starts_with(extract_names(search_term)[0])
    return unless search_term.count(' ').positive?
    where_clause << " AND #{case_insensitive_search(:last_name)}"
    where_args[:last_name] = starts_with(extract_names(search_term)[1])
  end

  def build_for_name_search(search_term)
    build_name_segment(search_term)
    @order = 'last_name asc'
  end
